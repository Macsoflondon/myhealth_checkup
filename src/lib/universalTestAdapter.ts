...
export function fromLegacyUnified(p: LegacyUnifiedProps): UniversalTestData {
  ...
  return {
    ...
    sample_type: fromDetails?.sample_type ?? p.collection,
    collection_method: fromDetails?.collection_method ?? null,
    measurement_type: fromDetails?.measurement_type ?? null,
    who_should_test: fromDetails?.who_should_test ?? null,
    biomarker_count: fromDetails?.biomarker_count ?? p.biomarkers,
    biomarkers_list:
