...
// ---------- NavLink (react-router-dom compat, incl. function-form props) ----------
...
export const NavLink = forwardRef<HTMLAnchorElement, NavLinkProps>(function NavLink(
  ...
  const target = parseTo(to).pathname.replace(/\/+$/, "") || "/";
  const isActive = end
    ? pathname === target
    : pathname === target || pathname.startsWith(target === "/" ? "/" : `${target}/`);
...
// ---------- MemoryRouter (test-only passthrough) ----------
// Unit tests wrap components in <MemoryRouter>; real routing context comes from
// the TanStack router in the app. This keeps test files type-checking until the
// test suite is re-wired to TanStack's test utilities.
