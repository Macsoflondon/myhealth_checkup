...
export default function AdminTestMapperPage() {
  ...
  const runMapper = async (dryRun: boolean) => {
    ...
    try {
      toast.info(
        `Starting AI Test Mapper in ${dryRun ? "DRY RUN" : "LIVE"} mode...`,
      );
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-white/90">
                  Automatically map 192 unmapped provider tests to master tests
                  using OpenAI GPT-5 semantic analysis
                </p>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">
                    {completionRate}% of tests mapped
                  </span>
                </div>
                <Progress value={completionRate} className="h-2" />
                <p className="text-xs text-on-dark-muted">
                  Target: 85%+ mapping rate (currently{" "}
                  {result.high_confidence_mapped}/{result.total_processed})
                </p>
              </div>
            )}
          </div>
          ...
          <div className="grid gap-6 mb-8">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Adjust mapping parameters for Day 1 launch blitz (75%
                  confidence threshold for maximum coverage)
                </CardDescription>
              </CardHeader>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Run the AI test mapper to automatically create mappings
                  between provider tests and master tests. Dry run mode previews
                  results without making database changes.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <div className="bg-primary/10 border border-primary/20 rounded-lg p-4">
                  ...
                  <div className="text-sm space-y-1 text-muted-foreground">
                    <p>
                      • <strong>Dry Run First:</strong> Preview results and
                      verify AI accuracy before committing
                    </p>
                    <p>
                      • <strong>Live Mapping:</strong> Creates actual mappings
                      in database (≥{confidenceThreshold}% confidence)
                    </p>
                    <p>
                      • <strong>Manual Review:</strong> 60-
                      {confidenceThreshold - 1}% confidence tests flagged for
                      review (export CSV)
                    </p>
                    <p>
                      • <strong>Target:</strong> Map 85%+ of 192 unmapped tests
                      (163+ successful mappings)
                    </p>
                    <p>
                      • Batch processing: {batchSize} tests per batch with 2s
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      ...
                      <div className="text-center p-4 bg-green-50 dark:bg-green-950 rounded-lg">
                        <div className="text-3xl font-bold text-green-600">
                          {result.high_confidence_mapped}
                        </div>
                        <div className="text-sm text-green-700 dark:text-green-400">
                          High Confidence
                        </div>
                        <div className="text-xs text-green-600 dark:text-green-500">
                          ≥{confidenceThreshold}% Auto-mapped
                        </div>
                      </div>
                      <div className="text-center p-4 bg-yellow-50 dark:bg-yellow-950 rounded-lg">
                        <div className="text-3xl font-bold text-yellow-600">
                          {result.medium_confidence_review}
                        </div>
                        <div className="text-sm text-yellow-700 dark:text-yellow-400">
                          Needs Review
                        </div>
                        <div className="text-xs text-yellow-600 dark:text-yellow-500">
                          60-{confidenceThreshold - 1}% Confidence
                        </div>
                      </div>
                      <div className="text-center p-4 bg-white rounded-lg">
                        ...
                        <div className="text-sm text-brand-navy dark:text-brand-navy">
                          Skipped
                        </div>
                        <div className="text-xs text-brand-navy dark:text-brand-navy">
                          &lt;60% Confidence
                        </div>
                      </div>
                    </div>
                ...
                {result.mappings_created.length > 0 && (
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>
                        {result.mappings_created.length} test mappings with ≥
                ...
                {result.review_needed.length > 0 && (
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>
                        {result.review_needed.length} tests require manual
                        verification (60-{confidenceThreshold - 1}% confidence)
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                        ...
                        {result.review_needed.map((item, idx) => (
                          ...
                          >
                            ...
                            {item.suggestions.map((suggestion, sIdx) => (
                              <div
                                key={sIdx}
                                className="text-xs pl-4 border-l-2 border-yellow-400"
                              >
                                <div className="font-medium">
                                  → {suggestion.master_test_name}
                                </div>
