    ...
    const fetchSportsTests = async () => {
      try {
        const { data, error } = await supabase
          .from('tests_master')
          .select('id, test_name, category, biomarkers')
          .or('category.ilike.%sport%,category.ilike.%performance%,category.ilike.%fitness%')
          .eq('is_active', true)
          .limit(6);
        ...
        if (data) {
          ...
          const { data: mappings } = await supabase
            ...
            .select('test_master_id, provider_id, current_price')
            .in('test_master_id', testIds)
  ...
  return (
    <>
      <Helmet>
        ...
        <meta 
          name="description" 
          content="Optimise your athletic performance with comprehensive sports blood tests. Monitor fitness biomarkers, nutrition status, and recovery markers from trusted UK providers. Compare prices and book today." 
        />
        <meta 
          name="keywords" 
          content="sports performance tests, athletic blood tests, fitness biomarkers, sports nutrition testing, endurance testing, recovery markers, athlete health screening, UK sports tests" 
        />
        ...
        <div className="container mx-auto px-4 py-4">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Sports Performance</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>
            ...
            <div className="max-w-4xl mx-auto text-center">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <Button 
                  ...
                  onClick={() => navigate('/compare?category=sports-performance')}
                >
                  Browse All Tests
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-7xl mx-auto">
              {sportsPerformanceCategories.map((category) => {
                ...
                return (
                  <Card 
                    key={category.id}
                    className="hover:shadow-xl transition-all duration-300 cursor-pointer border-2 hover:border-[#FA6980] group"
                  ...
                  >
                    ...
                    <CardContent>
                      ...
                      <Button 
                        variant="ghost" 
                        className="w-full group-hover:bg-[#FA6980] group-hover:text-white transition-colors"
                      >
                        View Tests
          ...
          <div className="container mx-auto px-4">
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-7xl mx-auto">
                {tests.map((test) => (
                  <Card key={test.id} className="hover:shadow-xl transition-all duration-300 border-2 hover:border-[#FA6980]">
                    <CardHeader>
                      <div className="flex justify-between items-start mb-2">
                        <Badge variant="secondary">{test.category}</Badge>
                        {test.price && (
                          <span className="text-2xl font-bold text-[#FA6980]">
                            £{test.price}
                          </span>
                        )}
                      </div>
                    ...
                    <CardContent>
                      <div className="space-y-3 mb-4">
                        <div className="flex items-center gap-2 text-sm text-muted-foreground">
                          ...
                          <span>
                            {test.biomarkers && typeof test.biomarkers === 'object' 
