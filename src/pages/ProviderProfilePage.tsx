...
const ProviderProfilePage = () => {
      ...
      <MainLayout>
        <main className="container mx-auto px-4 py-16">
          <div className="text-center">
            <PageHeading title="Provider" accent="Not Found" />
  ...
  const ratingData = providerRatingData
    ? {
        rating: providerRatingData.rating,
        reviews: providerRatingData.reviewsFormatted,
      }
    : null;
  const websiteUrl = provider.website
    ? buildProviderWebsiteUrl(provider.website, provider.id)
    : null;
  ...
  return (
    <MainLayout>
      <Helmet>
        ...
        <meta
          ...
          content={`${provider.name} private health tests reviewed and compared.${
            providerRatingData
              ...
              : ""
          } Browse the full test range, prices, accreditations and turnaround times.`.slice(
            0,
            158,
          )}
        />
        <link
          rel="canonical"
          href={`https://myhealthcheckup.co.uk/provider/${provider.id}`}
        />
        <meta property="og:type" content="website" />
        <meta property="og:site_name" content="myhealth checkup" />
        <meta
          property="og:title"
          content={`${provider.name} Reviews & Tests | myhealth checkup`}
        />
        <meta
          property="og:description"
          content={`${provider.name} private health tests reviewed and compared. Browse the full test range, prices and accreditations.`}
        />
        <meta
          property="og:url"
          content={`https://myhealthcheckup.co.uk/provider/${provider.id}`}
        />
      ...
      <main className="container mx-auto px-4 py-4 md:py-8">
          ...
          <div className="flex flex-col md:flex-row items-start gap-4 md:gap-6">
            ...
            >
              <ProviderLogo
                ...
                className="w-[4.5rem] h-[4.5rem] md:w-20 md:h-20 object-contain"
              />
            </div>
            ...
            <div className="flex-1 w-full">
              ...
              {brand && (
                <p className="text-white/90 text-sm md:text-base italic mb-2">
                  {brand.tagline}
                </p>
              )}
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-4 mb-4">
                {ratingData && (
                  <div className="flex items-center space-x-1">
                    ...
                    <span
                      ...
                      style={{
                        color: brand ? "#fff" : "hsl(var(--foreground))",
                      }}
                    >
                      {ratingData.rating}
                    </span>
                ...
                {provider.accreditation && (
                  <Badge
                    variant="secondary"
                    className="gap-1 text-xs bg-green-100 text-green-800"
                  >
                    <Shield className="w-3 h-3" />
                    Accredited
                  </Badge>
                )}
                {(provider as { cqcRegistrationNumber?: string })
        ...
        >
          <h2 className="font-semibold text-on-dark mb-4 flex items-center gap-2">
          ...
          <div className="grid sm:grid-cols-2 md:grid-cols-4 gap-4">
            <div className="flex items-center gap-3 p-3 bg-card rounded-lg">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {provider.accreditation || "UKAS ISO 15189"}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-3 p-3 bg-card rounded-lg">
              <Building2 className="w-5 h-5 flex-shrink-0 text-brand-navy" />
              <div>
                <p className="text-sm font-medium">CQC Regulated</p>
                <p className="text-xs text-muted-foreground">
                  {provider.clinics || "Registered clinics"}
                </p>
              </div>
            </div>
            {ratingData && (
            ...
            <div className="flex items-center gap-3 p-3 bg-card rounded-lg">
              ...
              <div>
                <p className="text-sm font-medium">
                  {provider.turnaroundTime || "2-5 days"}
                </p>
                <p className="text-xs text-muted-foreground">
                  Results turnaround
                </p>
                ...
                {provider.phone && (
                  <div className="min-h-[44px] flex flex-col justify-center">
                    ...
                    <a
                      ...
                      className="text-muted-foreground text-sm md:text-base hover:text-primary touch-manipulation"
                    >
                      {provider.phone}
                    </a>
                  </div>
                ...
                {provider.email && (
                  <div className="min-h-[44px] flex flex-col justify-center">
                    ...
                    <a
                      ...
                      className="text-muted-foreground text-sm md:text-base break-all hover:text-primary touch-manipulation"
                    >
                      {provider.email}
                    </a>
                  </div>
