    ...
    <section className="py-16 bg-gray-50">
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          <h2 className="text-4xl font-bold mb-4">Health Testing by Life Stage</h2>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Your health needs evolve with age. Discover the most important tests for your life stage and 
            how proactive monitoring can add healthy years to your future.
          </p>
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-4 mb-12">
          {ageGroups.map((group, index) => (
            <Button
              key={index}
              variant={selectedAge === index ? "default" : "outline"}
              size="lg"
              onClick={() => setSelectedAge(index)}
              className={`flex items-center gap-2 ${
                selectedAge === index ? 'bg-health-600 hover:bg-health-700' : ''
              }`}
            >
              {group.icon}
              Ages {group.range}
          ...
          <Card className="border-2 border-gray-200">
            <CardHeader className="text-center pb-6">
              <div className="flex items-center justify-center mb-4">
                <div className={`p-4 rounded-full ${ageGroups[selectedAge].color}`}>
                  {ageGroups[selectedAge].icon}
                </div>
              </div>
              <CardTitle className="text-3xl font-bold">
                {ageGroups[selectedAge].title}
              </CardTitle>
              <p className="text-lg text-gray-600 mt-2">
                {ageGroups[selectedAge].description}
              </p>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                  <h4 className="font-semibold text-lg mb-4 flex items-center">
                    <Heart className="w-5 h-5 mr-2 text-health-600" />
                    Recommended Tests
                  </h4>
                ...
                <div>
                  <h4 className="font-semibold text-lg mb-4 flex items-center">
                    <Users className="w-5 h-5 mr-2 text-health-600" />
                    Health Focus
                  </h4>
              ...
              <div className="flex justify-center mt-8">
                <Button size="lg" className="bg-health-600 hover:bg-health-700">
                  View Tests for Ages {ageGroups[selectedAge].range}
                </Button>
              </div>
            </CardContent>
          </Card>
