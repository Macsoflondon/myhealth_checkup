...
const BenefitCard = ({ icon, title, description, statistic, color, isActive, onClick }: BenefitCardProps) => {
  return (
    <Card 
      className={`cursor-pointer transition-all duration-300 transform hover:scale-105 ${
        isActive ? `border-2 ${color} shadow-lg` : 'border border-gray-200 hover:shadow-md'
      }`}
      onClick={onClick}
    >
      <CardContent className="p-6 text-center">
        <div className={`w-16 h-16 mx-auto mb-4 rounded-full flex items-center justify-center ${
          isActive ? color.replace('border-', 'bg-').replace('-500', '-100') : 'bg-gray-100'
        }`}>
          {icon}
        </div>
    ...
    <section className="py-16 bg-gradient-to-br from-blue-50 via-white to-green-50">
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          <h2 className="text-4xl font-bold mb-4 bg-gradient-to-r from-health-600 to-wellness-600 bg-clip-text text-transparent">
            The Power of Proactive Health
          </h2>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto mb-8">
            Discover how preventive health testing can transform your life and add healthy years to your future. 
            Click each benefit to learn more about the impact on your wellbeing.
          </p>
        </div>
        ...
        <div className="max-w-4xl mx-auto">
          <Card className="border-2 border-gray-100 bg-white/80 backdrop-blur-sm">
            <CardContent className="p-8">
              <div className="flex items-center justify-center mb-6">
                {benefits[activeCard].icon}
                <h3 className="text-2xl font-bold ml-4">{benefits[activeCard].title}</h3>
              </div>
