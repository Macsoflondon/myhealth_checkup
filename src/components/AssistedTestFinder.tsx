...
const getRecommendedTests = (concerns: string[], gender: string) => {
  ...
  const relevantTests = allTests.filter(test => concerns.some(concern => test.relevantFor.includes(concern)));
...
export const AssistedTestFinder = () => {
    ...
    return <div className="min-h-screen bg-gray-50">
        ...
        <div className="flex items-center justify-center min-h-[80vh] p-4">
          <div className="text-center max-w-2xl mx-auto">
            ...
            <Button onClick={handleGetStarted} className="bg-[#E91E63] hover:bg-[#C2185B] text-white px-12 py-4 text-lg font-medium rounded-full transition-colors">
              Get started
            </Button>
          </div>
        </div>
  ...
  if (currentStep === 'gender') {
    return <div className="min-h-screen bg-gradient-to-b from-pink-100 to-white">
        <UKASBanner />
        <Header />
        
        {/* Navigation Controls */}
        <div className="flex justify-between items-center p-6 max-w-6xl mx-auto">
          <Button onClick={handleBack} variant="outline" className="flex items-center gap-2 px-6 py-3 rounded-full border-gray-300">
            <ArrowLeft className="w-4 h-4" />
            Back
          </Button>
          <Button onClick={handleRestart} variant="outline" className="flex items-center gap-2 px-6 py-3 rounded-full border-pink-300 text-pink-600">
        ...
        <div className="flex items-center justify-center min-h-[60vh] p-4">
          <div className="text-center max-w-4xl mx-auto">
            ...
            <div className="flex flex-wrap justify-center gap-4 max-w-3xl mx-auto">
              {genderOptions.slice(0, 3).map(option => <Button key={option.id} onClick={() => handleGenderSelect(option.id)} className="bg-[#E91E63] hover:bg-[#C2185B] text-white px-8 py-4 text-lg font-medium rounded-full min-w-[180px] transition-colors">
                  {option.label}
                </Button>)}
            </div>
            
            <div className="mt-4 flex justify-center">
              <Button onClick={() => handleGenderSelect('prefer-not-to-say')} className="bg-[#E91E63] hover:bg-[#C2185B] text-white px-8 py-4 text-lg font-medium rounded-full min-w-[180px] transition-colors">
                Prefer not to say
              </Button>
            </div>
          </div>
        ...
        <div className="flex items-center justify-center min-h-[60vh] p-4">
          <div className="text-center max-w-5xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-12">
              {concernOptions.map(option => <Button key={option.id} onClick={() => handleConcernSelect(option.id)} variant={selectedConcerns.includes(option.id) ? 'default' : 'outline'} className={`px-6 py-4 text-lg font-medium rounded-full transition-colors ${selectedConcerns.includes(option.id) ? 'bg-[#E91E63] hover:bg-[#C2185B] text-white border-[#E91E63]' : 'border-[#E91E63] text-[#E91E63] hover:bg-[#E91E63] hover:text-white'}`}>
                  {option.label}
                </Button>)}
            </div>
            
            <Button onClick={handleContinue} className="bg-[#E91E63] hover:bg-[#C2185B] text-white px-12 py-4 text-lg font-medium rounded-full transition-colors disabled:opacity-50" disabled={selectedConcerns.length === 0}>
              Continue
            </Button>
          </div>
        </div>
  ...
  if (currentStep === 'results') {
    ...
    return <div className="min-h-screen bg-gradient-to-b from-pink-100 to-white bg-white">
        <Header />
        
        {/* Navigation Controls */}
        <div className="flex justify-between items-center p-6 max-w-6xl mx-auto">
          <Button onClick={handleBack} variant="outline" className="flex items-center gap-2 px-6 py-3 rounded-full border-gray-300 bg-[#22c0d4]">
            <ArrowLeft className="w-4 h-4" />
            Back
          </Button>
          <Button onClick={handleRestart} variant="outline" className="flex items-center gap-2 px-6 py-3 rounded-full border-pink-300 text-pink-600">
        ...
        <div className="max-w-6xl mx-auto p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            {recommendedTests.map(test => <div key={test.id} className="bg-white rounded-lg shadow-lg overflow-hidden border border-gray-200">
          ...
          <div className="text-center">
            <Button onClick={handleViewAllTests} className="bg-[#E91E63] hover:bg-[#C2185B] text-white px-8 py-3 text-lg font-medium rounded-full">
              View all tests
            </Button>
          </div>
        </div>
