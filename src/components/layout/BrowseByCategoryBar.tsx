...
import {
  CATEGORY_MENU_ICONS as ICONS,
  MORE_SECTION_ICONS,
  MENU_TURQUOISE as TURQUOISE,
  MENU_PINK as PINK,
...
import {
  LanguageList,
  LanguageAccordion,
} from "@/components/header/LanguageSwitcher";
...
export default function BrowseByCategoryBar({
  ...
  useEffect(() => {
    const compute = () => {
      ...
      const available =
        Math.min(bar.clientWidth || framedWidth, framedWidth) -
        padX -
        moreW -
        12;
  ...
  useLayoutEffect(() => {
    ...
    const find = () =>
      setAnchorEl(document.getElementById("page-toolbar-anchor"));
    find();
  ...
  useEffect(() => {
    ...
    const sync = () => {
      ...
      if (el) setMoreRect(el.getBoundingClientRect());
    };
    sync();
      ...
      : placement === "hero"
        ? `${
            heroPinned
              ...
              : "relative mx-0 w-full basis-full transition-[box-shadow] duration-300 ease-out motion-reduce:transition-none"
          } mt-0 min-w-0`
        : compact
          ? "mt-0 mx-3 lg:mx-6"
          : isFlush
  ...
  let innerClass =
    placement === "hero"
      ? `w-full max-w-full overflow-hidden bg-white border-x-0 border-[#22c0d4] ${heroPinned ? "border-t-2 border-b-2" : "border-t-0 border-b-0"}`
  ...
  return (
    <>
      ...
      <div
        ...
        aria-hidden="true"
        className={
          placement === "hero" ? "absolute inset-x-0 top-0 h-px" : "h-px w-full"
        }
      />
      <div
        ref={mobileBarRef}
        className="w-full shrink-0 basis-full relative"
        data-testid="browse-by-category-bar-mobile"
      >
        <div
          data-scrolled={scrolled}
          className={`${placement === "hero" ? "flex flex-col px-4 sm:px-6 md:px-9 min-h-[96px] md:min-h-[120px] py-3 md:py-4" : "flex items-center px-4 sm:px-6 md:px-9 h-24 md:h-[120px]"} transition-[background-color,border-color,box-shadow] duration-300 ease-out border-b-2 bg-white ${placement === "hero" ? "border-[#e70d69]" : "border-[#22c0d4]"}`}
        >
          <div className="flex items-start w-full shrink-0">
            <div className="flex flex-col min-w-0 pr-14">
              <Link
                to="/"
                className="flex items-center no-underline font-[Montserrat] font-extrabold tracking-[-0.02em] leading-none whitespace-nowrap text-[clamp(30px,6.6vw,88px)] py-1 md:py-2"
              >
                <span className="transition-colors duration-300 ease-out text-[#081129]">
                  myhealth
                </span>
                <span className="text-[#22c0d4]">checkup</span>
              </Link>
              {placement === "hero" && (
                <p className="m-0 mt-1 md:mt-2 text-left">
          ...
          <div
            className="absolute right-4 sm:right-6 md:right-9 lg:right-9 top-1/2 -translate-y-1/2 z-50"
            data-testid="mobile-floating-menu-trigger"
          >
            {/* Desktop: show the hamburger icon but keep it inactive */}
            <div
              className="hidden lg:flex flex-col items-end justify-center gap-[5px] min-w-11 min-h-11 p-2"
              aria-hidden="true"
            >
                    ...
                    <div className="space-y-6">
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-1 gap-2">
                          {items.map((item) => {
                            const { Icon, color } = ICONS[item.name] ?? {
                              Icon: Star,
                              color: TURQUOISE,
                            };
                        ...
                        <div key={section.title}>
                          ...
                          <div className="grid grid-cols-1 gap-2">
                            {section.items.map((item) => {
                              const { Icon, color } = MORE_SECTION_ICONS[
                                item.name
                              ] ?? { Icon: Info, color: TURQUOISE };
                              return (
                                <Link
                                  key={item.name}
                                  to={item.path}
                                  onClick={() => setMobileOpen(false)}
                                  className="rounded-xl bg-white border-[1.5px] border-[#081129]/10 group flex items-center gap-3 px-3 py-2.5 no-underline"
                                >
                                  <span
                                    className="w-8 h-8 rounded-full inline-flex items-center justify-center shrink-0"
                                    style={{ background: `${color}1a` }}
      ...
      {(() => {
        const desktopBar = (
          ...
          >
            <div
              className={`${placement === "hero" ? "px-2 sm:px-3 py-1.5" : "p-1.5"} transition-all duration-300 ${innerClass}`}
              data-testid="category-toolbar-dock"
            >
              <div
                className={`flex ${placement === "hero" ? "w-full justify-center" : "w-fit"} max-w-full min-w-0 items-center gap-1`}
              >
                <div
                  ref={stripRef}
                  className={`flex min-w-0 items-center gap-y-0 flex-nowrap [scrollbar-width:none] [&::-webkit-scrollbar]:hidden ${placement === "hero" ? "flex-none justify-center overflow-visible gap-x-1 xl:gap-x-1.5" : `flex-1 justify-start overflow-x-auto ${useStraddle ? "gap-x-0" : "gap-x-0 2xl:gap-x-1"}`}`}
                ...
                >
                  {items.map((item, index) => {
                    ...
                    return (
                      <div
                        key={item.name}
                        className={collapsed ? "hidden" : "shrink-0"}
                      >
                        <CategoryPillDropdown
                          ...
                          dense={placement === "hero" || useStraddle}
                        />
                      </div>
                    );
                  })}
                ...
                <div
                  ref={moreRef}
                  className={`relative shrink-0 ${placement === "hero" ? "ml-2" : ""}`}
                ...
                >
                  <button
                    ...
                    className={`group inline-flex items-center rounded-full transition-colors duration-200 ${moreOpen ? "bg-brand-pink/10" : "hover:bg-brand-pink/10"} ${placement === "hero" ? "gap-1 px-1 py-2" : useStraddle ? "gap-1.5 px-2.5 py-2" : "gap-0.5 px-1 py-2 2xl:gap-1 2xl:px-2 2xl:py-2.5"}`}
                  >
                    <span
                      className={`font-bold font-[Montserrat] whitespace-nowrap text-[#081129] group-hover:text-brand-pink ${placement === "hero" ? "text-[13px] xl:text-[14px] tracking-[-0.01em]" : useStraddle ? "text-[12.5px] lg:text-[13px] tracking-[-0.02em]" : "text-xs sm:text-[12px] tracking-[-0.02em] 2xl:text-sm 2xl:tracking-normal"}`}
                    >
                      More
                    </span>
                    <ChevronDown
                      className={`text-[#081129]/45 transition-transform duration-300 shrink-0 ${placement === "hero" ? "w-[12px] h-[12px]" : "w-[12px] h-[12px] 2xl:w-[14px] 2xl:h-[14px]"} group-hover:text-brand-pink ${moreOpen ? "rotate-180" : ""}`}
                    />
                  </button>
                  ...
                  {moreOpen &&
                    ...
                    createPortal(
                      <div
                        ...
                        style={{
                          top: moreRect ? moreRect.bottom + 8 : 0,
                          right: moreRect
                            ? Math.max(8, window.innerWidth - moreRect.right)
                            : 8,
                        }}
                      >
                        <MoreDropdownMenu
                          ...
                          languageList={
                            <LanguageList onSelect={() => setMoreOpen(false)} />
                          }
                        />
                      </div>,
                      document.body,
          ...
          return (
            <>
              ...
              <div
                ...
                style={{ height: heroPinned ? barHeight : 0 }}
              />
            </>
          );
        }
        return useStraddle && anchorEl
          ? createPortal(desktopBar, anchorEl)
          : desktopBar;
      })()}
    </>
  );
