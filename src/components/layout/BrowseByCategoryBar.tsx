  ...
  return (
    <>
      ...
      <div ref={mobileBarRef} className="w-full shrink-0 basis-full relative" data-testid="browse-by-category-bar-mobile">
        <div data-scrolled={scrolled} className={`${placement === "hero" ? "flex flex-col px-4 sm:px-6 md:px-9 min-h-[96px] md:min-h-[120px] py-3 md:py-4" : "flex items-center px-4 sm:px-6 md:px-9 h-24 md:h-[120px]"} transition-[background-color,border-color,box-shadow] duration-300 ease-out border-b-2 bg-white ${placement === "hero" ? "border-[#e70d69]" : "border-[#22c0d4]"}`}>
            <div className="flex items-start w-full shrink-0">
              <div className="flex flex-col min-w-0 pr-14">
                <Link to="/" className="flex items-center no-underline font-[Montserrat] font-extrabold tracking-[-0.02em] leading-none whitespace-nowrap text-[clamp(30px,6.6vw,88px)] py-1 md:py-2">
        ...
        const desktopBar = (
          ...
          >
            ...
            <div className={`${placement === "hero" ? "px-2 sm:px-3 py-1.5" : "p-1.5"} transition-all duration-300 ${innerClass}`} data-testid="category-toolbar-dock">
              <div className={`flex ${placement === "hero" ? "w-full justify-center" : "w-fit"} max-w-full min-w-0 items-center gap-1`}>

                <div ref={stripRef} className={`flex min-w-0 items-center gap-y-0 flex-nowrap [scrollbar-width:none] [&::-webkit-scrollbar]:hidden ${placement === "hero" ? "flex-none justify-center overflow-visible gap-x-1 xl:gap-x-1.5" : `flex-1 justify-start overflow-x-auto ${useStraddle ? "gap-x-0" : "gap-x-0 2xl:gap-x-1"}`}`} data-testid="category-pill-strip">
                ...
                <div ref={moreRef} className={`relative shrink-0 ${placement === "hero" ? "ml-2" : ""}`} data-testid="category-bar-right-cluster">
                  <button type="button" onClick={() => setMoreOpen((o) => !o)} aria-expanded={moreOpen} className={`group inline-flex items-center rounded-full transition-colors duration-200 ${moreOpen ? "bg-brand-pink/10" : "hover:bg-brand-pink/10"} ${placement === "hero" ? "gap-1 px-1 py-2" : useStraddle ? "gap-1.5 px-2.5 py-2" : "gap-0.5 px-1 py-2 2xl:gap-1 2xl:px-2 2xl:py-2.5"}`}><span className={`font-bold font-[Montserrat] whitespace-nowrap text-[#081129] group-hover:text-brand-pink ${placement === "hero" ? "text-[13px] xl:text-[14px] tracking-[-0.01em]" : useStraddle ? "text-[12.5px] lg:text-[13px] tracking-[-0.02em]" : "text-xs sm:text-[12px] tracking-[-0.02em] 2xl:text-sm 2xl:tracking-normal"}`}>More</span><ChevronDown className={`text-[#081129]/45 transition-transform duration-300 shrink-0 ${placement === "hero" ? "w-[12px] h-[12px]" : "w-[12px] h-[12px] 2xl:w-[14px] 2xl:h-[14px]"} group-hover:text-brand-pink ${moreOpen ? "rotate-180" : ""}`} /></button>
