        ...
        <div className={`${compact ? "px-2 py-2 sm:px-3 sm:py-2.5" : "px-2 sm:px-3 py-2.5 sm:py-3"} transition-all duration-300 ${innerClass}`}>
          <div className="flex items-center justify-center gap-x-1.5 gap-y-2 sm:gap-x-2 flex-nowrap max-w-full overflow-x-auto">
            ...
            <div ref={moreRef} className="relative shrink-0">
              <button type="button" onClick={() => setMoreOpen((o) => !o)} className={`inline-flex items-center rounded-full bg-white border-[1.5px] border-[#081129]/10 transition-all duration-200 ${compact ? "gap-1.5 pl-2.5 pr-3 py-2 sm:gap-2 sm:pl-3 sm:pr-3.5 sm:py-2.5" : "gap-1.5 pl-2.5 pr-3 sm:pl-3 sm:pr-3.5 py-2.5 sm:py-3"}`}><span className="rounded-full inline-flex items-center justify-center shrink-0 w-[22px] h-[22px] sm:w-[24px] sm:h-[24px]" style={{ background: "rgba(231,13,105,0.1)" }}><MoreHorizontal className="w-[13px] h-[13px] sm:w-[14px] sm:h-[14px]" style={{ color: "#e70d69" }} strokeWidth={2} /></span><span className="font-semibold text-[#081129] font-[Montserrat] text-[12px] sm:text-[13px] lg:text-[13.5px]">More</span><ChevronDown className={`text-[#081129]/60 transition-transform w-3.5 h-3.5 ${moreOpen ? "rotate-180" : ""}`} /></button>
