      ...
      <div className="md:hidden sticky top-0 z-50" data-testid="browse-by-category-bar-mobile">
        <div data-scrolled={scrolled} className={`px-4 h-20 flex items-center justify-between transition-[background-color,border-color,box-shadow] duration-300 ease-out border-b ${scrolled ? "bg-[#081129] border-[#081129] shadow-[0_2px_10px_rgba(8,17,41,0.18)]" : "bg-white border-[#081129]/10"}`}>
          ...
          <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
            <SheetTrigger asChild><button type="button" aria-label="Open menu" className={`inline-flex items-center justify-center w-11 h-11 rounded-full transition-colors duration-300 ease-out focus:outline-none ${scrolled ? "bg-white/10 text-white hover:bg-white/20" : "bg-[#081129]/5 text-[#081129] hover:bg-[#081129]/10"}`}><Menu className="w-6 h-6" strokeWidth={2.25} /></button></SheetTrigger>
        ...
        <div className={`${compact ? "px-2 py-2 sm:px-3 sm:py-2.5" : "px-2 sm:px-3 py-2.5 sm:py-3"} transition-all duration-300 ${innerClass}`}>
          <div className="flex items-center justify-center gap-x-1.5 gap-y-2 sm:gap-x-2 flex-wrap max-w-full">
            ...
            <div ref={moreRef} className="relative shrink-0">
              <button type="button" onClick={() => setMoreOpen((o) => !o)} className={`inline-flex items-center rounded-full bg-white border-[1.5px] border-[#081129]/10 transition-all duration-200 ${compact ? "gap-0.5 pl-1 pr-1.5 py-0.5" : "gap-1.5 pl-1.5 pr-2 py-1"}`}><span className={`rounded-full inline-flex items-center justify-center shrink-0 ${compact ? "w-[15px] h-[15px]" : "w-[18px] h-[18px]"}`} style={{ background: "rgba(231,13,105,0.1)" }}><MoreHorizontal className={`${compact ? "w-[9px]" : "w-[11px]"}`} style={{ color: "#e70d69" }} strokeWidth={2} /></span><span className={`font-semibold text-[#081129] font-[Montserrat] ${compact ? "text-[9.5px]" : "text-[11px]"}`}>More</span><ChevronDown className={`text-[#081129]/60 transition-transform ${moreOpen ? "rotate-180" : ""}`} /></button>
