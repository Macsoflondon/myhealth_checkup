...
async function resolveOne(rec: RecInput): Promise<UniversalTestData | null> {
  if (rec.actualTestId) {
    const { data } = await supabase
      ...
      .eq("is_active", true)
      .maybeSingle();
    if (data) return data as unknown as UniversalTestData;
  }
  ...
  return (data?.[0] as unknown as UniversalTestData) ?? null;
}
