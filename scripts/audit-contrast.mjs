...
const DARK_SURFACE = [
  /\bbg-\[#0[0-9a-f]{5}\](?!\/(?:\[?0\.0?\d|5|10)\]?)/i,
  /\bbg-\[#1[0-2][0-9a-f]{4}\](?!\/(?:\[?0\.0?\d|5|10)\]?)/i,
  /\bbg-navy\b/,
  /\bbg-brand-navy\b(?!\/(?:\[?0\.0?\d|5|10)\]?)/,
...
const LIGHT_SURFACE = [
  ...
  /\bbg-brand-turquoise/,
  /\bbg-\[#22c0d4\]/i,
  /\bbg-gradient-to-[a-z]{1,2}\b(?=[^"']*\bfrom-(?:\[#22c0d4\]|brand-turquoise))/,
  /\bbg-\[#[def][0-9a-f]{5}\]/i,
  /\bbg-\[#[A-F][0-9A-F]{5}\]/,
];
...
function hasLightInlineBackground(node) {
  const attrs = ts.isJsxElement(node)
    ? node.openingElement.attributes
    : node.attributes;
  ...
  return attrs.properties.some((attr) => {
    if (
      ...
      attr.name.getText() !== "style" ||
      !attr.initializer
    )
      return false;
    const text = attr.initializer.getText();
    return /background(?:Color)?\s*:\s*["'`](?:#fff(?:fff)?|white|rgba?\(255\s*,\s*255\s*,\s*255)/i.test(
      text,
    );
  });
}
  ...
  const visit = (node, darkSurface) => {
    ...
    if (ts.isJsxElement(node) || ts.isJsxSelfClosingElement(node)) {
      ...
      if (cls || tagName === "Card") {
        const light =
          tagName === "Card" ||
          hasLightInlineBackground(node) ||
          firstMatch(LIGHT_SURFACE, cls || "");
        ...
        if (nextDark) {
          ...
          const hasLightText = /\btext-white\b|\btext-\[#f[0-9a-f]{5}\]/i.test(
            cls,
          );
...
console.log(
  `Contrast audit: ${all.length} dark-on-dark hits in ${byFile.size} files\n`,
);
for (const [file, hits] of [...byFile.entries()].sort(
  ...
  for (const h of hits) {
    console.log(
      `  :${h.line}  ${h.token}  on  ${h.surface}  (line ${h.surfaceLine})`,
    );
  }
}
