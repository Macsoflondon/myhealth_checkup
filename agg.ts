...
async function run() {
  try {
    ...
    const canonCounts = (canonData || []).reduce((acc: any, curr: any) => {
      acc[curr.canonical_category] = (acc[curr.canonical_category] || 0) + 1;
      return acc;
    }, {});
    console.log('--- CANONICAL CATEGORY COUNTS ---');
    console.log(JSON.stringify(Object.entries(canonCounts).sort((a: any,b: any) => b[1] - a[1]), null, 2));
    ...
    const catCounts = (catData || []).reduce((acc: any, curr: any) => {
      acc[curr.category] = (acc[curr.category] || 0) + 1;
      return acc;
    }, {});
    console.log('\n--- CATEGORY COUNTS ---');
    console.log(JSON.stringify(Object.entries(catCounts).sort((a: any,b: any) => b[1] - a[1]), null, 2));
    ...
    const distinctSections = [...new Set((sectionData || []).map((d: any) => d.source_section))];
    ...
    const filteredBowel = (bowelData || []).filter((t: any) => 
    ...
    ).filter((t: any) => terms.some(term => t.test_name?.toLowerCase().includes(term)));
