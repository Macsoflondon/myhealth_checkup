...
const BASE_URL = 'https://www.clinilabs.com';
...
async function firecrawlMap(url: string, apiKey: string): Promise<string[]> {
  const response = await fetch('https://api.firecrawl.dev/v1/map', {
    method: 'POST',
    headers: { 'Authorization': `Bearer ${apiKey}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({ url, search: 'blood test health check', limit: 200, includeSubdomains: false }),
  });
  if (!response.ok) throw new Error(`Firecrawl map error: ${response.status}`);
  const data = await response.json();
  // Filter to product/test pages — adjust pattern based on site structure
  return (data.links || []).filter((l: string) =>
    (l.includes('/product') || l.includes('/test') || l.includes('/blood-test') || l.includes('/health-check'))
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('Starting Clinilabs Firecrawl scraper...');
    ...
    // If map didn't find much, try scraping the homepage for links
      ...
      try {
        ...
        if (homeResult.success && homeResult.data?.markdown) {
          ...
          for (const m of urlMatches) {
            if (m[1].includes(BASE_URL) && (m[1].includes('test') || m[1].includes('product') || m[1].includes('health'))) {
    ...
    for (const url of productUrls.slice(0, 50)) {
      try {
        ...
        let title = metadata.title?.replace(/\s*[–|]\s*Clinilabs.*$/i, '').trim() || '';
        if (!title) {
        ...
        products.push({
          ...
          provider_test_id: `clinilabs-${slug}`,
          category: determineCategory(title, metadata.description || ''),
          price,
          description: metadata.description || `${title} from Clinilabs.`,
          url,
          is_active: true,
    ...
    return new Response(JSON.stringify({
      success: true, message: `Scraped ${products.length} Clinilabs tests`, testsUpdated: products.length,
    }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });

  } catch (error) {
    console.error('Clinilabs scraper error:', error);
