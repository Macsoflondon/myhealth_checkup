...
| P0.02 | All migrations reviewed | IN PROGRESS | Set difference computed exactly: 126 applied with no committed file, of which 12 are `+1s` timestamp near-misses and **114 are true orphans**. All 114 classified from `schema_migrations.statements`: 62 catalogue DML, 17 security, 13 schema-bearing, 8 other, rest views/cron/functions/index. Fully attributable — every row carries `name`, `created_by` and verbatim SQL. Outstanding: backfill the schema-bearing set, and fix `migration-parity.yml`, whose remote step is guarded by a `secrets` expression that step-level `if` cannot evaluate — worklist W2 |
...
| P0.08 | Legacy and duplicate table review | IN PROGRESS | Duplicates enumerated with row counts and column lists, and a keep/retire recommendation recorded for each: `profiles`/`user_profiles` (resolved, see P0.05); `user_consents`/`clinical_consent_records` (extend the clinical one); `biomarker_readings`/`clinical_biomarker_history`/`test_results` (retire all three, all empty). `uploaded_test_results` holds the only 2 rows of real user health data — proposed migration into `clinical_patient_uploads` plus new `observations`. **No decision ratified by the owner yet** |
| P0.09 | Architecture gap report | IN PROGRESS | Mapping of all 40 canonical entities against the live schema is complete: 9 exist and are usable, 7 exist but are unsuitable or empty, **5 are name collisions**, 19 are missing. Observation contract checked field by field — no existing table supplies more than ~half. Outstanding: ratification of the 5 collision decisions (`biomarkers`/`biomarker_hub`, `audit_logs`, `ai_prompt_versions`, `consent_records`, `observations`/`reference_ranges`) — worklist W6 |
| P0.10 | Security review | IN PROGRESS | RLS and storage policies evidenced. **New finding: `public.profiles` has RLS policies but zero grants** — confirming the grant layer diverges from the policy layer and must be audited live (W3). Outstanding: `test-results` bucket has no file-size limit and no MIME allow-list; no test asserts the `<uid>/` prefix invariant; auth password/MFA policy, backups/PITR, cron inventory and live grants are dashboard-only and unverified — worklist W3, W4, W5 |
