...
**Clinical-safety patterns adopted from Forth's Terms of Service** (their contract is more informative than their marketing): abnormal-result escalation is a **standing duty, not a consent toggle** — their clause 11.5 overrides the customer's own opt-out where a result is abnormal by generally accepted UK medical standards, so ours needs its own lawful basis, a deterministic trigger on validated observations, a human in the loop and an audit trail, and must never be AI-triggered (P0-e); previous-result context is a safety requirement for reviewers because practitioner continuity is not guaranteed (P1-e); a non-diagnostic position and a named-symptom emergency exclusion must be visible at the point of viewing a result (P0-h).
...
| 14 Sep 2026 (sixth pass) | Full-lifecycle Forth Connect competitive analysis across all six published surfaces including Solutions and the Terms of Service. `docs/FORTH_CONNECT_COMPETITIVE_ARCHITECTURE.md` rewritten: what Forth actually operates, six-way separation (features / architecture / clinical governance / commercial workflow / deliberate refusals / where we must be stronger), eighteen-layer mapping to canonical entities, and a P0/P1/P2 backlog with dependencies, acceptance criteria and do-not-build-yet items. Three clinical-safety patterns adopted from their Terms, chiefly abnormal-result escalation as a standing duty rather than a preference. Master plan gained §7 (eight areas where we go further) and §8 (Terms-derived safety patterns). `build-tracker.json` gained `competitiveBacklog.priorities`, `doNotBuildYet`, `refusals` and `strongerThanForth`. P0-g shipped as `canonical-neutrality.test.ts`, which caught and removed a partner reference in the canonical contract file. No schema change, no migration, no marketplace change. |
